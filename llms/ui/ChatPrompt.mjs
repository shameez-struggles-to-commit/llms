...
import { processSSEStream, accumulateChunks } from './streaming.mjs'
...
export default {
    template:`
    <div class="mx-auto max-w-3xl" data-testid="chat-prompt">
        ...
        <div class="flex space-x-2">
            ...
            <div class="flex-1">
                ...
                <div v-if="attachedFiles.length" class="mt-2 flex flex-wrap gap-2" data-testid="attachments">
                    <div v-for="(f,i) in attachedFiles" :key="i" class="flex items-center gap-2 px-2 py-1 rounded-md border border-gray-300 dark:border-gray-600 text-xs text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800" :data-testid="'attachment-item-' + i">
                ...
                <div v-if="!model" class="mt-2 text-sm text-red-600 dark:text-red-400" data-testid="no-model-warning">
        ...
        const sendMessage = async () => {
            ...
            try {
                ...
                if (isStreaming) {
                    ...
                    const createStreamingMessage = async () => {
                        const tempMessage = {
                            role: 'assistant',
                            content: ''
                        }
                        await threads.addMessageToThread(threadId, tempMessage)
                        const thread = await threads.getThread(threadId)
                        streamingMessageId = thread.messages[thread.messages.length - 1].id
                    ...
                    try {
                        ...
                        // Process SSE stream
                        await processSSEStream(response, async (chunk) => {
                            chunks.push(chunk)
                            ...
                            if (chunk.choices && chunk.choices[0] && chunk.choices[0].delta) {
                                ...
                                if (delta.content) {
                                    ...
                                    await threads.updateMessageContent(threadId, streamingMessageId, streamingContent)
                                }
                            }
                        }, controller.signal)

                        // Accumulate final result from all chunks
                        result = accumulateChunks(chunks)
                        
                        // Ensure the final message content matches
                        if (result && result.choices && result.choices[0]) {
                            result.choices[0].message.content = streamingContent
                        }

                    } catch (error) {
                        // Remove the temporary streaming message on error
                        if (streamingMessageId) {
                            await threads.deleteMessage(threadId, streamingMessageId)
                        }
                        throw error
                    }
                ...
                if (!errorStatus.value && result) {
                    ...
                    // For streaming, the message is already added; for non-streaming, add it now
                    if (!isStreaming) {
                        await threads.addMessageToThread(threadId, assistantMessage, usage)
                    } else {
                        // Update the final message with usage information
                        if (usage) {
                            await threads.updateMessageUsage(threadId, result.choices[0].message.content, usage)
                        }
                    }
