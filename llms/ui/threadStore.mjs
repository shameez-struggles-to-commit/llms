...
async function deleteMessage(threadId, messageId) {
    return await deleteMessageFromThread(threadId, messageId)
}

async function updateMessageContent(threadId, messageId, content) {
    const thread = await getThread(threadId)
    if (!thread) throw new Error('Thread not found')
...
async function updateMessageUsage(threadId, messageContent, usage) {
    const thread = await getThread(threadId)
    if (!thread) throw new Error('Thread not found')

    // Find message by content (last assistant message with this content)
    const messageIndex = thread.messages.slice().reverse().findIndex(m => m.content === messageContent && m.role === 'assistant')
    if (messageIndex === -1) throw new Error('Message not found')

    const actualIndex = thread.messages.length - 1 - messageIndex
    const updatedMessages = [...thread.messages]
    ...
    // Also update previous user message with input tokens
    if (actualIndex > 0 && usage?.prompt_tokens != null) {
        const prevMessage = updatedMessages[actualIndex - 1]
    ...
    const stats = await calculateThreadStats(threadId)
    await updateThread(threadId, { messages: updatedMessages, stats })
}
